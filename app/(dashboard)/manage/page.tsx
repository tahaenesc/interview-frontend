...
function CategoryList({ type }: { type: TransactionType }) {
  ...
  const dataAvailable = categoriesQuery.data && categoriesQuery.data.legth > 0;
  ...
  return (
    <SkeletonWrapper isLoading={categoriesQuery.isFetching}>
      <Card>
        ...
        <Separator />
        {!dataAvailable && (
          <div className="flex h-40 q-full flex-col items-center justify-center">
            <p>
              {" "}
              No{" "}
              <span
...
function CategoryCard({ category }: { category: Category }) {
  return (
    <div className="flex border-seperate flex-col justify-between rounded-md border shadow-md shadow-black/[0.1] dark:shadow-white/[0.1] ">
      ...
      <Button
        className="flex w-full border-seperate items-center gap-2 rounded-t-none taxt-muted-foreground hover:bg-red-500/20"
        variant={"secondary"}
      >
